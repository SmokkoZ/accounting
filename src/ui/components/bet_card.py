...
def _render_matching_suggestions(
    suggestions: MatchingSuggestions, max_items: int = 3
) -> None:
...
def _render_event_suggestion(suggestion: EventSuggestion) -> None:
    ...
    match_label = f"Match: {suggestion.similarity:.0f}%"
    detail_parts: List[str] = [match_label]
    if suggestion.reason:
        detail_parts.append(suggestion.reason)
    if suggestion.hours_apart is not None:
        detail_parts.append(f"Time delta: {suggestion.hours_apart:.1f}h")

    message = f"{suggestion.name}\n\n" + " | ".join(detail_parts)
...
def _render_market_suggestion(suggestion: MarketSuggestion) -> None:
    ...
    header = f"{suggestion.market_code} ({suggestion.description})"
    detail_parts: List[str] = [f"Match: {suggestion.score:.0f}%"]
    if suggestion.period_scope:
        detail_parts.append(f"Period: {suggestion.period_scope}")
    if suggestion.line_value:
        detail_parts.append(f"Line: {suggestion.line_value}")
    if suggestion.side:
        detail_parts.append(f"Side: {suggestion.side}")
    if suggestion.reason:
        detail_parts.append(suggestion.reason)
