...
- [x] Task 1: Create centralized caching system (AC: 1)
  - [x] Create `src/ui/cache.py` with `@st.cache_resource` for DB connections
  - [x] Create `@st.cache_data(ttl=5)` wrapper for query results
  - [x] Implement `query_df(sql, params=())` function for cached DataFrame queries
  - [x] Add cache invalidation helpers for write operations
  - [x] Add unit tests for caching functionality

- [x] Task 2: Implement pagination system (AC: 2)
  - [x] Create pagination helper functions in `src/ui/utils/pagination.py`
  - [x] Implement `LIMIT/OFFSET` SQL generation with parameterized queries
  - [x] Add total count queries with caching
  - [x] Create pagination UI components (25/50/100 options, next/prev)
  - [x] Add per-table session state management for pagination
  - [x] Integrate pagination into all major tables (incoming bets, surebets, ledger, etc.)
  - [x] Add unit tests for pagination logic

- [x] Task 3: Create thumbnail system for images (AC: 3)
  - [x] Create `src/ui/media.py` with `make_thumb(path, w=300)` function
  - [x] Implement thumbnail generation with PIL and caching
  - [x] Update list views to show thumbnails instead of full images
  - [x] Add click-to-expand functionality for full image viewing
  - [x] Implement thumbnail cache management
  - [x] Add unit tests for thumbnail generation and display

- [x] Task 4: Implement form-gated filters (AC: 4)
  - [x] Update incoming bets page filters to use `st.form`
  - [x] Update verified bets page filters to use `st.form`
  - [x] Update reconciliation page filters to use `st.form`
  - [x] Ensure filter changes don't trigger immediate reruns
  - [x] Add clear filters functionality
  - [x] Test form-gated filters prevent mid-typing reruns

- [x] Task 5: Create rerun guardrails system (AC: 5)
  - [x] Implement `safe_rerun(reason)` helper in `src/ui/utils/state_management.py`
  - [x] Add debug logging for rerun reasons
  - [x] Remove ad-hoc `st.rerun()` calls from render loops
  - [x] Replace with safe rerun patterns where needed
  - [x] Add unit tests for rerun guardrails

- [x] Task 6: Optimize hot paths (AC: 6)
  - [x] Audit code for `sleep` calls in hot paths
  - [x] Remove or optimize unnecessary sleep calls
  - [x] Audit for noisy debug logs in performance-critical sections
  - [x] Implement conditional logging based on debug flags
  - [x] Add performance monitoring for hot path identification

- [x] Task 7: Create and apply database indexes (AC: 7)
  - [x] Create migration script for performance indexes
  - [x] Add `idx_bets_status_created` index
  - [x] Add `idx_surebets_status_created` index
  - [x] Add `idx_ledger_surebet` index
  - [x] Add `idx_ledger_created` index
  - [x] Verify index usage with `EXPLAIN` queries
  - [x] Add unit tests for index migration

- [x] Task 8: Create performance dashboard (AC: 8)
  - [x] Add performance timing collection to UI pages
  - [x] Create perf dashboard in Admin → Advanced section
  - [x] Show last 50 page load timings with breakdowns
  - [x] Create "UI Performance Playbook" documentation
  - [x] Add performance monitoring helpers
  - [x] Add integration tests for performance dashboard

- [x] Task 9: Implement performance benchmarks (AC: 9)
  - [x] Create benchmark test suite for 10k bet dataset
  - [x] Implement filter performance measurement (≤ 1.5s target)
  - [x] Test expander/popover responsiveness
  - [x] Add performance regression tests
  - [x] Document benchmark results and targets
...
def display_thumbnail_or_full(image_path: str, caption: str = "", width: int = 300):
    ...
    if thumb_data:
        if st.image(thumb_data, caption=caption, width="content"):
    ...
    if st.session_state.get(f"show_full_{image_path}"):
        ...
        st.image(image_path, caption=f"Full Size: {caption}", width="stretch")
```
...
def safe_rerun(reason: str = "User action", max_frequency: float = 0.5):
    ...
    if st.session_state.get("debug_mode", False):
        st.sidebar.write(f"🔄 Rerun: {reason} at {time.ctime(current_time)}")
    ...
    with st.expander("⚡ Performance Dashboard", expanded=False):
        ...
        with col1:
            ...
            with st.expander("Database Performance"):
                ...
                st.write("📊 Database metrics coming soon")
        ...
        with col2:
            ...
            with st.expander("Cache Performance"):
                ...
                st.write("💾 Cache metrics coming soon")
...
│   ├── test_media.py                    # Thumbnail generation tests
│   ├── test_performance_helpers.py       # Rerun guardrails tests
│   └── test_database_indexes.py         # Index migration tests
├── integration/
│   ├── test_performance_integration.py   # End-to-end performance tests
│   └── test_benchmark_scenarios.py     # Large dataset benchmarks
└── performance/
    ├── benchmark_10k_bets.py           # 10k bet performance test
    └── performance_regression.py       # Performance regression suite
```
...
def test_10k_bet_filter_performance():
    """Benchmark: 10k bets → filter apply ≤ 1.5s"""
...
```
src/ui/
├── cache.py                           # NEW: Centralized caching system
├── media.py                           # NEW: Thumbnail and media handling
├── utils/
│   ├── pagination.py                   # NEW: Pagination helpers
│   └── state_management.py            # ENHANCED: Rerun guardrails
└── pages/
    ├── 1_incoming_bets.py              # UPDATED: Pagination + caching
    ├── 2_surebets.py                 # UPDATED: Pagination + caching
    ├── 3_settlement.py                # UPDATED: Pagination + caching
    ├── 4_reconciliation.py            # UPDATED: Pagination + caching
    ├── 5_export.py                    # UPDATED: Pagination + caching
    ├── 6_statements.py                # UPDATED: Pagination + caching
    └── 7_admin_associates.py         # UPDATED: Performance dashboard
```

**Database Migration Files** [Source: docs/architecture/source-tree.md#scripts/]:
```
scripts/
├── migrate_performance_indexes.sql      # NEW: Performance indexes migration
└── benchmark_10k_data.py             # NEW: Performance test data generator
```
...
tests/
├── unit/
│   ├── test_cache.py                    # Unit tests for caching system
│   ├── test_pagination.py                # Unit tests for pagination logic
│   ├── test_media.py                    # Unit tests for thumbnail generation
│   ├── test_performance_helpers.py       # Unit tests for rerun guardrails
│   └── test_database_indexes.py         # Unit tests for index migration
├── integration/
│   ├── test_performance_integration.py   # Integration tests for performance optimizations
│   ├── test_benchmark_scenarios.py     # Integration tests for large datasets
│   └── test_performance_dashboard.py    # Tests for performance monitoring
├── performance/
│   ├── benchmark_10k_bets.py           # Performance benchmark for 10k dataset
│   └── performance_regression.py       # Performance regression test suite
└── fixtures/
    ├── performance_test_data.sql         # Test data for performance scenarios
    └── sample_screenshots/             # Sample images for thumbnail testing
```
...
# tests/performance/benchmark_10k_bets.py
def test_10k_bet_filter_performance():
    """Benchmark: Filter 10k bets in ≤ 1.5 seconds"""
...
def test_10k_bet_pagination_performance():
    """Benchmark: Paginate 10k bets in ≤ 0.5 seconds"""
...
- [ ] Filter application times meet performance targets (≤ 1.5s for 10k records)
...
| 2025-11-08 | 1.1 | Added benchmark suite, performance alerts, and QA regression tests | James (Dev) |
...
- `pytest tests/unit/test_performance_utils.py tests/unit/test_media_thumbnails.py tests/unit/test_pagination_utils.py tests/unit/test_statement_service.py tests/performance tests/integration/test_performance_dashboard.py -q`

### Completion Notes List
- Delivered `scripts/benchmark_10k_bets.py` plus the new `tests/performance` suite to exercise the 10k bet dataset and enforce the ≤1.5s filter budget.
- Added performance alert buffers + helper utilities so the Admin dashboard now surfaces threshold violations with reset controls and documented the workflow in the playbook.
- Expanded regression coverage for pagination, thumbnails, performance helpers, and statement calculations to close the QA coverage gaps and replace the broken ad-hoc test script.
- Added deterministic dashboard data helpers and integration tests, then refreshed story metadata/status for QA handoff.

### File List
- docs/performance-playbook.md
- docs/stories/8.9.performance-hardening-caching-pagination-thumbnails-db-indexes.md
- scripts/benchmark_10k_bets.py
- src/ui/pages/7_admin_associates.py
- src/ui/utils/performance.py
- src/ui/utils/performance_dashboard.py
- tests/integration/test_performance_dashboard.py
- tests/performance/__init__.py
- tests/performance/test_benchmark_10k_bets.py
