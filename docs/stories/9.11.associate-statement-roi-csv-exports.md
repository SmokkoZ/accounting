# Story 9.11: Associate Statement & ROI Excel Exports - Brownfield Addition
...
1. `StatementService.export_statement_excel(associate_id, as_of_utc)` produces Statement Excel with:
...
2. `StatementService.export_surebet_roi_excel(associate_id, as_of_utc)` produces Per-Surebet ROI Excel with surebet_id, settled timestamp, associate stake/profit/ROI%, and group stake/profit/ROI% for fully settled surebets only.
...
6. ROI Excel groups handle VOID legs and multi-leg surebets, skip/blank ROI when stake is zero, and exclude partially settled surebets (or mark them clearly per UX note).
...
## Tasks / Subtasks
- [x] Implement `StatementService.export_statement_excel`/`export_surebet_roi_excel` helpers that reuse existing calculators, add ROI aggregation queries, enforce cutoff filtering, and emit slugged filenames with export-only rounding.
...
- [x] Expand `tests/integration/test_statement_flow.py` to validate per-bookmaker target math, footer totals, and ROI Excel division/VOID handling; run the suite to cover regressions.
...
- Statement/ROI Excels align to the YF identity: include `YF = ND + FS` in summaries and add an “Exit Payout” row (`−Δ`) when exporting at an exit cutoff.
