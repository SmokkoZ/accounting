...
- [x] (New) Post-approval editing support:
  - An operator can edit already-approved (status="verified") bets from a dedicated queue
  - On save, system logs MODIFIED audit entry and re-attempts surebet matching if the bet remains "verified"
  - The edit view shows screenshot preview and latest raw OCR extraction for context
...
|------|---------|-------------|--------|
| 2025-11-01 | 1.2 | Added Verified Bets Queue with post-approval editing and re-matching | Codex Agent |
...
✓ Ready for Done
 - [x] Task 8: Add Verified Bets Queue page
   - [x] Create `src/ui/pages/3_verified_bets_queue.py` with filters and table view
   - [x] Display selected bet context (screenshot + raw OCR) and provide edit form
   - [x] Wire to `BetVerificationService.update_verified_bet()` to persist edits and re-attempt matching
