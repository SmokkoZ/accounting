...
- [x] Task 10: Add EUR formatting utilities (AC: 2)
  ...
  - [x] Ensure consistent EUR display (€XX.XX format)
...
def render_surebets_page():
    st.title("🎯 Verified Bets & Surebets")
...
- **Safe (✅)**: `worst_case_profit_eur >= 0` AND `roi >= 1.0%` → Green badge
- **Low ROI (🟡)**: `worst_case_profit_eur >= 0` BUT `roi < 1.0%` → Yellow badge
- **Unsafe (❌)**: `worst_case_profit_eur < 0` → Red badge with prominent styling
...
Gate: PASS → docs/qa/gates/3.3-surebet-safety-dashboard-ui.yml
