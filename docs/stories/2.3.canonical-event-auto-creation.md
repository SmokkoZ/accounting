...
## Story
**As the system**, I want to automatically create canonical events from normalized OCR event names with minimal operator input, even when kickoff time is missing, so matching (Epic 3) has the required `canonical_event_id` without extra work.

## Acceptance Criteria
- [x] Event name is normalized consistently across the system
  - Normalization rules unify separators (v / vs. / - / : / @) to "vs"
  - Collapse whitespace and title-case team tokens
  - Implemented in `EventNormalizer.normalize_event_name()`
- [x] When operator approves bet (Story 2.2) with `canonical_event_id = NULL`, the system attempts to set it automatically:
  - Input fields: normalized `event_name`, `sport` (optional), `competition` (optional), `kickoff_time_utc` (optional)
  - If `kickoff_time_utc` is present and valid:
    - Perform fuzzy matching among events within ±24h window; threshold 80%
    - If matched, reuse existing event; else create a new one
  - If `kickoff_time_utc` is missing or invalid (common case):
    - Compute pair key from normalized name by splitting teams around "vs" and slugifying
    - Try exact `(sport, pair_key)` match first
    - If not found, perform exact (case-insensitive) normalized-name match, then strict fuzzy (≥ 90)
    - If none match, create a new event with `kickoff_time_utc = NULL` and store pair key columns
...
- [x] If no match found, create new `canonical_events` row with normalized fields:
  - `INSERT INTO canonical_events (normalized_event_name, sport, league, team1_slug, team2_slug, pair_key, kickoff_time_utc)`
...
- [x] Validation:
  - event_name: Required, min 5 chars (normalized before validation)
  - sport: Required when creating new events; if omitted, defaults to `football`
  - kickoff_time_utc: Optional (may be NULL); if provided, must be ISO8601 with `Z`
...
- [x] Task 3.1: Add relaxed path without kickoff time (AC update)
  - [x] Implement exact normalized-name lookup (case-insensitive) by `event_name` (+ optional `sport`)
  - [x] Create canonical event with NULL kickoff when no match and no kickoff available
- [x] Task 3.2: Normalize event names centrally (AC update)
  - [x] Add `EventNormalizer` with deterministic rules
  - [x] Use it in both OCR and approval flows
- [x] Task 3.3: Auto-create on OCR success (configurable)
  - [x] If OCR confidence ≥ threshold (default 0.90), create/match canonical event immediately after extraction
  - [x] Config flags: `AUTO_CREATE_EVENT_ON_OCR` and `OCR_EVENT_CONFIDENCE_THRESHOLD`
 - [x] Task 4: Add pair-key logic for matching and storage
   - [x] Split normalized name into teams → slugify → compute `pair_key = min|max`
   - [x] Store `team1_slug`, `team2_slug`, `pair_key` when creating canonical events
   - [x] Add `(sport, pair_key)` index and use it as first match step
 - [x] Task 5: Externalize team aliases
   - [x] Create `data/team_aliases.json` with language/format variants
   - [x] Load aliases in `EventNormalizer` to unify tokens before pair-key computation
...
|------|---------|-------------|--------|
| 2025-11-01 | 1.3 | Introduced pair-key matching (team slugs) + alias file; kickoff optional path refined | Codex Agent |
| 2025-11-01 | 1.2 | Normalized name-first event creation; kickoff optional; OCR auto-create with config | Codex Agent |
