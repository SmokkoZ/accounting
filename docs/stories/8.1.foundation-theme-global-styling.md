...
- [x] Task 5: Replace deprecated `use_column_width` parameter usage (AC: 5)
  ...
  - [x] Replace with `use_container_width=True`
...
- Fallbacks for ≥ 1.30 via feature flags
...
tests/
├── unit/
│   ├── test_ui_components.py          # Unit tests for ui_components.py helpers
│   └── test_theme_integration.py      # Tests for theme loading and application
└── integration/
    └── test_global_styling_flow.py    # Integration tests for styling across pages
```
...
- 2025-11-06: `pytest tests/unit/test_ui_components.py -q` ✅
- 2025-11-06: `pytest` (full suite) ⏱️ timed out after 20s; existing Windows temp-file locks triggered fixture teardown failures
