...
3. **Partner-Facing Section (Shareable):**
   ...
   - Display: "You funded: €X,XXX.XX total" with explanation
   ...
   - Display: "You're entitled to: €X,XXX.XX" with explanation
...
4. **Internal-Only Section (NOT for partners):**
   ...
   - Display: "Currently holding: €X,XXX.XX" with explanation
   - Reconciliation Delta: `DELTA = CURRENT_HOLDING_EUR - SHOULD_HOLD_EUR`
   - Display delta status with color coding: 🔴 Holding more, 🟢 Balanced, 🟠 Short
...
- [ ] Create Monthly Statements page UI structure (AC: 1)
  - [ ] Add new page file: `src/ui/pages/6_statements.py`
  - [ ] Implement associate selector dropdown from database
  - [ ] Add cutoff date picker with default to end of current month
  - [ ] Implement "Generate Statement" button with proper styling
  - [ ] Add page to main navigation in `src/ui/app.py`
  - [ ] Apply consistent UI patterns from existing pages

- [ ] Implement statement calculation service logic (AC: 2, 3, 4)
  - [ ] Create `src/services/statement_service.py`
  - [ ] Implement NET_DEPOSITS_EUR calculation (DEPOSIT - WITHDRAWAL)
  - [ ] Implement SHOULD_HOLD_EUR calculation (principal_returned_eur + per_surebet_share_eur)
  - [ ] Implement CURRENT_HOLDING_EUR calculation (all ledger entries)
  - [ ] Implement RAW_PROFIT_EUR and DELTA calculations
  - [ ] Add cutoff date filtering to all queries (WHERE created_at_utc <= cutoff)

- [ ] Format and display statement sections (AC: 2, 3, 4)
  - [ ] Create statement header with associate name, cutoff date, generated timestamp
  - [ ] Format currency values with commas (e.g., "€1,234.56")
  - [ ] Round to 2 decimal places for display
  - [ ] Implement profit/loss color coding (green for profit, red for loss)
  - [ ] Implement delta status indicators (🔴🟢🟠) with explanations
  - [ ] Add explanatory text for each calculation as specified

- [ ] Add export functionality (AC: 5)
  - [ ] Implement "Copy to Clipboard" for partner-facing section
  - [ ] Add "Export to CSV" for detailed transaction list
  - [ ] Prepare foundation for future "Export to PDF" functionality
  - [ ] Implement proper CSV formatting for associate-specific transactions

- [ ] Add validation and error handling (AC: 6, 7)
  - [ ] Validate cutoff date is not in future
  - [ ] Ensure associate is selected before generation
  - [ ] Add read-only constraint enforcement (no ledger writes)
  - [ ] Handle database errors gracefully
  - [ ] Log statement generation for audit trail

- [ ] Add comprehensive testing (Backend & UI)
  - [ ] Unit tests for statement service calculation logic
  - [ ] Integration tests for complete statement generation workflow
  - [ ] Test with various associate scenarios (profitable, loss, balanced)
  - [ ] Test cutoff date filtering and edge cases
  - [ ] UI testing for form interactions and display formatting
...
- Format with commas for display: "€1,234.56"
...
- Add page to main navigation with icon 📊
